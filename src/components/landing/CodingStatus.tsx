...
export default function CodingStatus() {
  ...
  useEffect(() => {
    ...
    // Refresh every 10 minutes (server response is cached for 5, so this
    // won't cause extra WakaTime calls — it just keeps the UI reasonably fresh)
  ...
  // Tap toggles on touch devices; auto-hides after 3s
  const handleClick = () => {
    clearHideTimeout();
    setShowDetails((prev) => {
      if (prev) return false;
      hideTimeout.current = setTimeout(() => setShowDetails(false), 3000);
      return true;
    });
  };
